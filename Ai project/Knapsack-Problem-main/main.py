        ...
        for _ in range(self.generations):
            ...
            for i in range(len(self.population)):
                if self.unbounded:
                    ...
                    self.population[i] = self.mutation(self.population[i], self.mutation_probability)
...
class KnapsackSolverGUI:
    def __init__(self, root):
        ...
        # Add labels for displaying best fitness and accuracy
        self.best_fitness_label = ttk.Label(root, text="Best Fitness:", font=self.font_style)
        self.best_fitness_display = ttk.Label(root, text="", font=self.font_style)
        #self.best_fitness_label.grid(row=8, column=0, sticky=tk.W)
        self.best_fitness_display.grid(row=8, column=1, columnspan=3, sticky=tk.W + tk.E)
    ...
    def solve_knapsack(self):
        try:
            ...
            for generation in range(generations):
                ...
                # Update the displayed best fitness value
                self.best_fitness_display.config(text=f"Best Fitness (Generation {generation+1}): {total_value}")

            # Calculate the accuracy as a percentage of the maximum possible value
            max_possible_value = sum(values)
            accuracy_percentage = (total_value / max_possible_value) * 100
            ...
            result_str = (
                ...
                f"Value: {total_value}\n"
                f"Accuracy: {accuracy_percentage:.2f}% of the maximum possible value"
            )
